...
class HTLargeChemicalReactorCategory(key: HTMachineKey) : HTMachineCategoryBase(key) {
    override fun arrangeSlots(display: HTMachineRecipeDisplay, bounds: Rectangle, consumer: Consumer<Widget>) {
        ...
        createSlot(consumer, bounds, 1, 2, display.getFluidInput(3), true)
        createSlot(consumer, bounds, 2, 2, display.getFluidInput(4), true)
        // outputs
        ...
        createSlot(consumer, bounds, 5, 2, display.getFluidOutput(3), false)
        createSlot(consumer, bounds, 6, 2, display.getFluidOutput(4), false)
        // catalyst
        createSlot(consumer, bounds, 3.5, 1.0, display.catalyst, true)
    }

    override val maxSlots: Int = 3
}
